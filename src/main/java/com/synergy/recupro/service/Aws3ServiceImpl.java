...
//								documents.add(saveMetaData(multipartFile, id, documentBody));
							} catch (Exception e) {
								e.printStackTrace();
							}
						}
	 ...
	 */
	public Document saveMetaData(MultipartFile file, Long id, Document documentBody) throws IOException {
		Document metaData = new Document();
		if (id != null) {
			Candidate candidate = candidateRepository.findByCandidateId(id);
			metaData.setCandidate(candidate);
			metaData.setDocumentName(documentBody.getDocumentName());
			metaData.setDocumentType(documentBody.getDocumentType());
			metaData.setIssueDate(documentBody.getIssueDate());
			metaData.setExpiryDate(documentBody.getExpiryDate());
			metaData.setNotes(documentBody.getNotes());
